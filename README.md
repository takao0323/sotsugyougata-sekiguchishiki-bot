...
「卒業型ダイエットメンター」は、あなた専属のAIメンターが、温かく前向きなメッセージでダイエットをサポートする専用プログラムです。期間終了時には感動的な卒業セレモニーで、あなたの努力と成果を讃えます。
...
- **2つのダイエットモード**：ライトダイエット（月3%減量）とハードダイエット（月5%減量）から選択可能
...
2. **プロフィール入力**
   - 名前、性別、年齢、身長
   - **活動レベル選択**：5段階から選択
   - **ダイエットモード選択**：ライトダイエット（月3%）またはハードダイエット（月5%）
   - 現在の体重、目標体重
3. **科学的な目標設定**
...
7. **🎓 卒業セレモニー**（プラン期間終了時）
   - **表彰状の授与**：達成度と成果を表彰
   - **紆余曲折のドラマ**：期間中の体重変化を振り返る感動的なメッセージ
   - 今後へのエールと応援メッセージ
...
- **注意点**：5%を超えると体が飢餓状態と判断し、代謝が低下する可能性
...
feedback_{名前}_{日時}.json                # 卒業時のフィードバック（JSON形式）
feedback_{名前}_{日時}.txt                 # 卒業時のフィードバック（テキスト形式）
...
このプロジェクトは、科学的根拠に基づく健康的なダイエットと、前向きなコーチングスタイルの重要性を信じて作成されました。
