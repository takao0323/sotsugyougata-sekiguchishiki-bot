#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
関口貴夫監修 関口式ダイエットメンター
世界一の実績と、圧倒的な成功実例を持つ関口式ダイエットメンタープログラムを完全再現
"""

import random
import os
from food_analyzer import analyze_food_image, get_image_path_from_user
import matplotlib.pyplot as plt
import matplotlib
# 日本語フォント設定
matplotlib.rcParams['font.family'] = 'DejaVu Sans'


def print_header():
    """プログラム起動時のヘッダーを表示"""
    print("=" * 60)
    print("💪 関口貴夫監修 関口式ダイエットメンター 💪")
    print("=" * 60)
    print("世界一の実績と、圧倒的な成功実例を持つ")
    print("関口式ダイエットメンタープログラムを完全再現")
    print("=" * 60)
    print()
    print("✨ 科学的根拠×指導歴30年の経験 ✨")
    print()
    print("ダイエットの現場では「理論はこうだけど、現実は違うよね」")
    print("ということがたくさんあります。")
    print("そんな現場のノウハウを全て詰め込みました。")
    print()
    print("-" * 60)
    print("【関口貴夫 経歴】")
    print("-" * 60)
    print("• 指導歴30年")
    print("• 健康体力づくり事業財団認定・健康運動指導士")
    print("• IFBBアーノルドクラシック世界大会優勝")
    print("• 文部科学大臣表彰")
    print("• JBBFオールジャパン優勝×2回")
    print("• ジャパンカップ優勝")
    print("• JBBF関東オーバーオール優勝他多数")
    print("-" * 60)
    print()


def get_meal_examples():
    """
    コンビニ飯と自炊のメニュー例を返す

    Returns:
        dict: カロリー帯別のメニュー例
    """
    return {
        "convenience": [
            {
                "name": "サラダチキン＋おにぎり（梅）＋野菜サラダ",
                "calories": 420,
                "protein": 32,
                "fat": 6,
                "carbs": 58,
                "description": "定番の高タンパク・低脂質メニュー"
            },
            {
                "name": "ゆで卵×2＋焼き魚おにぎり＋野菜スティック",
                "calories": 380,
                "protein": 28,
                "fat": 12,
                "carbs": 42,
                "description": "タンパク質豊富で腹持ちが良い"
            },
            {
                "name": "プロテインバー＋バナナ＋ギリシャヨーグルト",
                "calories": 350,
                "protein": 25,
                "fat": 8,
                "carbs": 45,
                "description": "間食や朝食におすすめ"
            },
            {
                "name": "鯖の塩焼き＋もち麦おにぎり＋温野菜",
                "calories": 480,
                "protein": 35,
                "fat": 15,
                "carbs": 50,
                "description": "オメガ3も摂れる栄養満点メニュー"
            },
            {
                "name": "豆腐バー＋鮭おにぎり＋カットフルーツ",
                "calories": 400,
                "protein": 28,
                "fat": 10,
                "carbs": 52,
                "description": "植物性・動物性タンパク質のバランス◎"
            },
            {
                "name": "チキン＋ブロッコリー弁当＋プロテイン",
                "calories": 550,
                "protein": 48,
                "fat": 12,
                "carbs": 60,
                "description": "筋トレ後に最適な高タンパクメニュー"
            },
            {
                "name": "焼き鳥（塩）×5本＋おにぎり＋海藻サラダ",
                "calories": 520,
                "protein": 42,
                "fat": 14,
                "carbs": 55,
                "description": "低脂質で満足感のあるメニュー"
            },
            {
                "name": "ツナサラダ＋玄米おにぎり＋ゆで卵",
                "calories": 450,
                "protein": 35,
                "fat": 10,
                "carbs": 50,
                "description": "オフィスランチに最適"
            },
            {
                "name": "サバ缶＋もち麦ご飯＋キャベツの千切り",
                "calories": 490,
                "protein": 38,
                "fat": 16,
                "carbs": 48,
                "description": "コスパ◎で栄養価も高い"
            },
            {
                "name": "蒸し鶏＋五穀米おにぎり＋きのこスープ",
                "calories": 410,
                "protein": 32,
                "fat": 8,
                "carbs": 52,
                "description": "低カロリーで栄養バランス◎"
            }
        ],
        "homemade": [
            {
                "name": "鶏胸肉のグリル200g＋玄米150g＋ブロッコリー",
                "calories": 520,
                "protein": 48,
                "fat": 10,
                "carbs": 58,
                "description": "ダイエットの王道メニュー"
            },
            {
                "name": "鮭のムニエル＋さつまいも＋ほうれん草のお浸し",
                "calories": 480,
                "protein": 38,
                "fat": 14,
                "carbs": 52,
                "description": "オメガ3と食物繊維が豊富"
            },
            {
                "name": "豚ヒレ肉の生姜焼き＋雑穀米＋野菜たっぷり味噌汁",
                "calories": 510,
                "protein": 42,
                "fat": 12,
                "carbs": 55,
                "description": "ビタミンB群が豊富で疲労回復に◎"
            },
            {
                "name": "豆腐ハンバーグ＋玄米＋野菜サラダ",
                "calories": 450,
                "protein": 35,
                "fat": 10,
                "carbs": 50,
                "description": "植物性タンパク質でヘルシー"
            },
            {
                "name": "白身魚の蒸し物＋もち麦ご飯＋わかめスープ",
                "calories": 420,
                "protein": 40,
                "fat": 6,
                "carbs": 52,
                "description": "超低脂質で消化にも優しい"
            },
            {
                "name": "牛もも肉のステーキ150g＋玄米＋温野菜",
                "calories": 540,
                "protein": 45,
                "fat": 15,
                "carbs": 50,
                "description": "鉄分・亜鉛も摂れる"
            },
            {
                "name": "鶏むね肉の親子丼風＋もずく酢＋野菜スープ",
                "calories": 490,
                "protein": 42,
                "fat": 10,
                "carbs": 58,
                "description": "卵でタンパク質をさらに強化"
            },
            {
                "name": "サバの味噌煮＋五穀米＋小松菜のお浸し",
                "calories": 500,
                "protein": 38,
                "fat": 16,
                "carbs": 48,
                "description": "DHA・EPAで脳にも良い"
            },
            {
                "name": "鶏ささみの棒々鶏＋玄米＋キノコのスープ",
                "calories": 440,
                "protein": 45,
                "fat": 8,
                "carbs": 50,
                "description": "超高タンパク・超低脂質"
            },
            {
                "name": "イカと野菜の炒め物＋もち麦ご飯＋豆腐の味噌汁",
                "calories": 460,
                "protein": 35,
                "fat": 10,
                "carbs": 55,
                "description": "低カロリーでボリューム満点"
            }
        ]
    }


def suggest_meal_examples(target_calories, target_protein, target_fat, target_carbs, name):
    """
    ユーザーの目標カロリー・PFCに合わせたメニュー例を提案

    Args:
        target_calories (float): 目標カロリー
        target_protein (float): 目標タンパク質
        target_fat (float): 目標脂質
        target_carbs (float): 目標炭水化物
        name (str): ユーザー名
    """
    meals = get_meal_examples()

    # カロリー範囲（±100kcal）でフィルタリング
    suitable_convenience = [
        meal for meal in meals["convenience"]
        if abs(meal["calories"] - target_calories) <= 150
    ]
    suitable_homemade = [
        meal for meal in meals["homemade"]
        if abs(meal["calories"] - target_calories) <= 150
    ]

    # フィルタリング結果が少ない場合は全て表示
    if len(suitable_convenience) < 3:
        suitable_convenience = meals["convenience"][:5]
    if len(suitable_homemade) < 3:
        suitable_homemade = meals["homemade"][:5]

    print("\n" + "=" * 60)
    print("🍱 関口からのメニュー提案")
    print("=" * 60)
    print(f"\n{name}さん、目標カロリー{target_calories:.0f}kcalに合わせた")
    print("具体的なメニュー例をご紹介します！\n")
    print("これらを参考に、無理なく続けていきましょう💪\n")

    # コンビニ飯
    print("【🏪 コンビニで揃えられるメニュー】")
    print("-" * 60)
    for i, meal in enumerate(suitable_convenience[:5], 1):
        print(f"\n{i}. {meal['name']}")
        print(f"   📊 {meal['calories']}kcal | P{meal['protein']}g F{meal['fat']}g C{meal['carbs']}g")
        print(f"   💡 {meal['description']}")

    print("\n")

    # 自炊
    print("【🍳 自炊で作れるメニュー】")
    print("-" * 60)
    for i, meal in enumerate(suitable_homemade[:5], 1):
        print(f"\n{i}. {meal['name']}")
        print(f"   📊 {meal['calories']}kcal | P{meal['protein']}g F{meal['fat']}g C{meal['carbs']}g")
        print(f"   💡 {meal['description']}")

    print("\n" + "=" * 60)
    print("💡 ポイント")
    print("=" * 60)
    print(f"• 1日の目標: {target_calories:.0f}kcal")
    print(f"• PFC目標: P{target_protein:.0f}g / F{target_fat:.0f}g / C{target_carbs:.0f}g")
    print("• 3食に分けて、バランスよく食べましょう")
    print("• 完璧を目指さなくて大丈夫！±10%の範囲ならOKです")
    print("=" * 60)
    print()


def detect_dropout_risk(profile, weight_history, daily_records, day_count):
    """
    挫折リスクを検知する

    Args:
        profile (dict): プロフィール情報
        weight_history (list): 体重履歴
        daily_records (list): 日々の記録
        day_count (int): 現在の日数

    Returns:
        dict: リスク情報 {
            "risk_level": "low" | "medium" | "high",
            "reasons": [理由のリスト],
            "suggestions": [提案のリスト]
        }
    """
    risk_level = "low"
    reasons = []
    suggestions = []

    # 1. カロリー超過の連続を検知（直近3日）
    if len(daily_records) >= 3:
        recent_records = daily_records[-3:]
        over_count = sum(1 for r in recent_records if r.get("calories", 0) > profile["target_calories"])

        if over_count == 3:
            risk_level = "high"
            reasons.append("3日連続でカロリー超過が続いています")
            suggestions.append("完璧を目指さなくて大丈夫！まずは目標の±10%以内を目指しましょう")
            suggestions.append("今日は「体重を測る」「記録する」だけでもOKです")
        elif over_count == 2:
            risk_level = "medium"
            reasons.append("カロリー超過が続いています")
            suggestions.append("無理に減らさなくて大丈夫。まずは現状維持を目指しましょう")

    # 2. 停滞期の長期化を検知（直近7日）
    if len(weight_history) >= 8:  # 最低8日必要（初日含む）
        recent_weights = weight_history[-8:]
        initial = recent_weights[0]
        final = recent_weights[-1]
        weight_change = initial - final

        # 7日間で0.3kg未満の変化 = 停滞期
        if abs(weight_change) < 0.3:
            if risk_level == "low":
                risk_level = "medium"
            reasons.append("体重が1週間以上停滞しています")
            suggestions.append("停滞期は成長のサインです。焦らず継続しましょう")
            suggestions.append("水分摂取、睡眠、ストレス管理を見直してみましょう")

        # 7日間で増加傾向 = 要注意
        elif weight_change < -0.5:
            risk_level = "high"
            reasons.append("体重が増加傾向にあります")
            suggestions.append("一時的な増加は誰にでもあります。諦めないでください")
            suggestions.append("基本に立ち返りましょう：カロリー・PFC・運動・睡眠")

    # 3. PFCバランスの乱れを検知（直近3日）
    if len(daily_records) >= 3:
        recent_records = daily_records[-3:]
        pfc_issues = 0

        for record in recent_records:
            protein = record.get("protein", 0)
            if protein and protein < profile["target_protein"] * 0.7:  # タンパク質70%未満
                pfc_issues += 1

        if pfc_issues >= 2:
            if risk_level == "low":
                risk_level = "medium"
            reasons.append("タンパク質の摂取量が不足しています")
            suggestions.append("筋肉を維持するため、タンパク質をしっかり摂りましょう")
            suggestions.append("プロテイン、サラダチキン、卵など手軽なものでOKです")

    # 4. 記録の継続性チェック（day_countと記録数の比較）
    if len(daily_records) < day_count - 1:  # 記録が抜けている
        missing_days = day_count - 1 - len(daily_records)
        if missing_days >= 2:
            risk_level = "high"
            reasons.append(f"記録が{missing_days}日分抜けています")
            suggestions.append("完璧じゃなくて大丈夫！体重だけでも記録しましょう")
            suggestions.append("継続することが一番大切です")

    return {
        "risk_level": risk_level,
        "reasons": reasons,
        "suggestions": suggestions
    }


def provide_special_support(risk_info, profile):
    """
    挫折リスクに応じた特別サポートを提供

    Args:
        risk_info (dict): リスク情報
        profile (dict): プロフィール情報
    """
    if risk_info["risk_level"] == "low":
        return  # リスク低い場合は何もしない

    print("\n" + "⚠️" * 30)
    print()

    if risk_info["risk_level"] == "high":
        print("【🚨 関口からの緊急メッセージ 🚨】")
    else:
        print("【💡 関口からのアドバイス 💡】")

    print("=" * 60)
    print(f"\n{profile['name']}さん、")
    print()

    # 理由を表示
    for reason in risk_info["reasons"]:
        print(f"  ❗ {reason}")

    print()
    print("でも、大丈夫です。")
    print("こういう時期は誰にでもあります。")
    print()
    print("【🌟 今すぐできること】")
    print("-" * 60)

    # 提案を表示
    for i, suggestion in enumerate(risk_info["suggestions"], 1):
        print(f"\n{i}. {suggestion}")

    print()
    print("-" * 60)
    print()

    if risk_info["risk_level"] == "high":
        print("💪 ハードルを下げましょう！")
        print()
        print("   ✅ 今日は「体重を測る」だけでOK")
        print("   ✅ カロリー計算は明日から")
        print("   ✅ とにかく「続ける」ことが最優先")
        print()
        print("   👉 続けることができれば、それだけで100点です！")
    else:
        print("💪 小さな一歩から始めましょう！")
        print()
        print("   できることから、一つずつ。")
        print("   あなたなら大丈夫です。")

    print()
    print("=" * 60)
    print("継続は力なり。")
    print("一緒に乗り越えましょう！")
    print("=" * 60)
    print()
    print("⚠️" * 30)
    print()



def calculate_bmr(gender, weight, height, age):
    """
    ハリス・ベネディクト方式（改良版）で基礎代謝量（BMR）を計算

    Args:
        gender (str): 性別（"男性" または "女性"）
        weight (float): 体重（kg）
        height (float): 身長（cm）
        age (int): 年齢

    Returns:
        float: 基礎代謝量（kcal/日）
    """
    if gender == "男性":
        # 男性: (13.397×体重kg)+(4.799×身長cm)-(5.677×年齢)+88.362
        bmr = (13.397 * weight) + (4.799 * height) - (5.677 * age) + 88.362
    else:  # 女性
        # 女性: (9.247×体重kg)+(3.098×身長cm)-(4.330×年齢)+447.593
        bmr = (9.247 * weight) + (3.098 * height) - (4.330 * age) + 447.593

    return bmr


def calculate_tdee(bmr, activity_coefficient):
    """
    総消費カロリー（TDEE）を計算

    Args:
        bmr (float): 基礎代謝量（kcal/日）
        activity_coefficient (float): 活動レベル係数

    Returns:
        float: 総消費カロリー（kcal/日）
    """
    return bmr * activity_coefficient


def create_nutrition_graphs(name, actual_calories, target_calories, actual_protein, target_protein,
                           actual_fat, target_fat, actual_carbs, target_carbs):
    """
    カロリーとPFCの比較グラフを生成

    Args:
        name (str): ユーザー名
        actual_calories (float): 実際のカロリー
        target_calories (float): 目標カロリー
        actual_protein (float): 実際のタンパク質
        target_protein (float): 目標タンパク質
        actual_fat (float): 実際の脂質
        target_fat (float): 目標脂質
        actual_carbs (float): 実際の炭水化物
        target_carbs (float): 目標炭水化物

    Returns:
        str: グラフファイルのパス
    """
    # 2つのサブプロットを作成（左：カロリー、右：PFC）
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    # === 左側：カロリー比較グラフ ===
    categories = ['Calories']
    actual_values = [actual_calories]
    target_values = [target_calories]

    x = range(len(categories))
    width = 0.35

    bars1 = ax1.bar([i - width/2 for i in x], actual_values, width, label='Actual', color='#FF6B6B', alpha=0.8)
    bars2 = ax1.bar([i + width/2 for i in x], target_values, width, label='Target', color='#4ECDC4', alpha=0.8)

    ax1.set_ylabel('kcal', fontsize=12)
    ax1.set_title('Calorie Comparison', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ax1.set_xticklabels(categories)
    ax1.legend()
    ax1.grid(axis='y', alpha=0.3)

    # 値をバーの上に表示
    for bar in bars1:
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height,
                f'{int(height)}',
                ha='center', va='bottom', fontsize=10)
    for bar in bars2:
        height = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2., height,
                f'{int(height)}',
                ha='center', va='bottom', fontsize=10)

    # === 右側：PFC比較グラフ ===
    if actual_protein is not None and actual_fat is not None and actual_carbs is not None:
        pfc_categories = ['Protein (g)', 'Fat (g)', 'Carbs (g)']
        actual_pfc = [actual_protein, actual_fat, actual_carbs]
        target_pfc = [target_protein, target_fat, target_carbs]

        x2 = range(len(pfc_categories))

        bars3 = ax2.bar([i - width/2 for i in x2], actual_pfc, width, label='Actual', color='#FF6B6B', alpha=0.8)
        bars4 = ax2.bar([i + width/2 for i in x2], target_pfc, width, label='Target', color='#4ECDC4', alpha=0.8)

        ax2.set_ylabel('grams', fontsize=12)
        ax2.set_title('PFC Comparison', fontsize=14, fontweight='bold')
        ax2.set_xticks(x2)
        ax2.set_xticklabels(pfc_categories)
        ax2.legend()
        ax2.grid(axis='y', alpha=0.3)

        # 値をバーの上に表示
        for bar in bars3:
            height = bar.get_height()
            ax2.text(bar.get_x() + bar.get_width()/2., height,
                    f'{height:.1f}',
                    ha='center', va='bottom', fontsize=10)
        for bar in bars4:
            height = bar.get_height()
            ax2.text(bar.get_x() + bar.get_width()/2., height,
                    f'{height:.1f}',
                    ha='center', va='bottom', fontsize=10)

    plt.tight_layout()

    # ファイル名を生成
    filename = f"nutrition_comparison_{name}.png"
    filepath = os.path.join(os.getcwd(), filename)

    # グラフを保存
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()

    return filepath


def get_stagnation_advice_templates():
    """
    体重停滞・増加時のアドバイステンプレート集

    Returns:
        dict: カテゴリ別のアドバイステンプレート
    """
    return {
        # 導入部分（励まし）- 25パターン
        "introduction": [
            "大丈夫、焦らなくていいんです。",
            "こういう時期は誰にでもあります。",
            "体重が停滞するのは、体が適応している証拠です。",
            "ここが踏ん張りどころですね。",
            "停滞期は成長のサインでもあるんですよ。",
            "体重は日々変動するものです。",
            "一時的な増加に一喜一憂する必要はありません。",
            "こんな日もあります。それが人間ですから。",
            "完璧じゃなくていいんです。",
            "今までよく頑張ってきましたね。",
            "ここまで継続してきたこと、それ自体が素晴らしいです。",
            "停滞は前進の一部です。",
            "体は正直です。今は調整期間なんです。",
            "諦めないことが一番大切です。",
            "この壁を越えれば、また変化が訪れます。",
            "停滞期こそ、本当の力がつく時期なんです。",
            "焦りは禁物です。じっくりいきましょう。",
            "体重計の数字だけが全てじゃありません。",
            "継続していれば必ず変化は訪れます。",
            "今は体が次のステージの準備をしているんです。",
            "ここで諦めたら、今までの努力がもったいないですよ。",
            "停滞期を乗り越えた先に、本当の成果があります。",
            "こういう時こそ、基本に立ち返りましょう。",
            "体重は減る時も停滞する時もあります。それが自然です。",
            "今日より明日、明日より明後日。少しずつ前に進んでいきましょう。",
        ],

        # 水分摂取のアドバイス - 25パターン
        "hydration": [
            "💧 水分摂取を意識してください。脂肪を分解するには水が不可欠です。一緒に頑張りましょう。",
            "💧 1日2リットルを目標に、こまめに水を飲みましょう。応援していますよ。",
            "💧 起床後すぐにコップ1杯の水を飲む習慣をつけてみてください。あなたならできます。",
            "💧 食事の30分前に水を飲むと、代謝が上がりやすくなります。そばで見守っていますよ。",
            "💧 冷たい水より常温の水の方が体に優しいですよ。いつも応援しています。",
            "💧 水分不足は代謝を下げます。意識して飲んでください。一緒に乗り越えましょう。",
            "💧 トイレの色をチェック。薄い黄色なら水分OK、濃いなら水分不足です。信じていますよ。",
            "💧 運動前後は特に水分補給を忘れずに。応援していますよ。",
            "💧 喉が渇く前に飲む。これが理想的な水分補給です。一緒に頑張りましょう。",
            "💧 レモン水やハーブティーもおすすめです。あなたならできます。",
            "💧 水筒を持ち歩いて、こまめに飲む習慣をつけましょう。そばにいますよ。",
            "💧 1時間にコップ1杯を目安に飲んでみてください。応援していますよ。",
            "💧 水分が足りないと、体は水を溜め込もうとします。たくさん飲みましょう。一緒に頑張りましょう。",
            "💧 炭酸水も良いですが、無糖のものを選んでくださいね。見守っていますよ。",
            "💧 体重×30mlが1日の目安です。しっかり飲みましょう。応援していますよ。",
            "💧 水を飲むことで満腹感も得られます。いつもそばにいますよ。",
            "💧 朝、昼、夜、各食事前に必ずコップ1杯の水を。一緒に頑張りましょう。",
            "💧 カフェインは利尿作用があるので、その分多めに水を。応援していますよ。",
            "💧 水分は代謝の潤滑油です。ケチらずに飲みましょう。信じていますよ。",
            "💧 緑茶やウーロン茶も良いですが、水がベストです。そばで見守っていますよ。",
            "💧 脱水状態では脂肪は燃えません。水が重要なんです。一緒に乗り越えましょう。",
            "💧 入浴前後にもしっかり水分補給を。応援していますよ。",
            "💧 スマホにリマインダーを設定して、定期的に飲む習慣を。あなたならできます。",
            "💧 水を飲むことで老廃物の排出も促進されます。いつも見守っていますよ。",
            "💧 体が水で満たされていると、代謝が活発になります。一緒に頑張りましょう。",
        ],

        # サプリメントのアドバイス - 25パターン
        "supplements": [
            "💊 マルチビタミン・ミネラルで栄養バランスを整えましょう。応援していますよ。",
            "💊 ビタミンB群は代謝をサポートします。一緒に頑張りましょう。",
            "💊 マグネシウムが不足すると代謝が落ちます。サプリも検討を。そばにいますよ。",
            "💊 ビタミンDは脂肪燃焼に重要です。見守っていますよ。",
            "💊 亜鉛は筋肉の維持に必要不可欠です。あなたならできます。",
            "💊 オメガ3脂肪酸（魚油）も代謝アップに効果的です。応援していますよ。",
            "💊 ビタミンCは抗酸化作用があり、回復を助けます。信じていますよ。",
            "💊 鉄分不足は疲労の原因に。特に女性は意識を。そばにいますよ。",
            "💊 カルシウムも代謝に関わる重要なミネラルです。応援していますよ。",
            "💊 プロテインパウダーでタンパク質を補うのも良いですね。一緒に頑張りましょう。",
            "💊 ビタミンEは血行促進に役立ちます。見守っていますよ。",
            "💊 コエンザイムQ10は細胞のエネルギー産生を助けます。あなたならできます。",
            "💊 カリウムは浮腫み防止に効果的です。応援していますよ。",
            "💊 ビタミンB12は疲労回復に重要です。一緒に乗り越えましょう。",
            "💊 葉酸も代謝サイクルに欠かせません。そばで見守っていますよ。",
            "💊 L-カルニチンは脂肪燃焼をサポートします。応援していますよ。",
            "💊 食事だけでは不足しがちな栄養素をサプリで補いましょう。信じていますよ。",
            "💊 質の良いマルチビタミンを1つ選ぶだけでも違います。一緒に頑張りましょう。",
            "💊 亜鉛・マグネシウム・ビタミンB6のZMAも人気です。あなたならできます。",
            "💊 ビタミンKは骨の健康にも重要です。見守っていますよ。",
            "💊 セレンは抗酸化ミネラルとして働きます。応援していますよ。",
            "💊 ビオチンは代謝を助けるビタミンです。そばにいますよ。",
            "💊 クロムは血糖値の安定に役立ちます。一緒に頑張りましょう。",
            "💊 サプリメントは補助です。食事が基本であることを忘れずに。応援していますよ。",
            "💊 質の良いサプリを選ぶことが大切です。いつも見守っていますよ。",
        ],

        # 運動のアドバイス - 25パターン
        "exercise": [
            "🏃 運動の強度を少し上げてみるのも良いかもしれません。大変だけど、やってみよう！",
            "🏃 5分でも10分でも有酸素運動を取り入れると効果的ですよ。応援していますよ。",
            "🏃 筋トレで筋肉量を増やすと基礎代謝が上がります。きついけど、挑戦してみましょう！",
            "🏃 ウォーキングでも十分効果があります。毎日30分を目標に。大変だけど、やってみよう！",
            "🏃 インターバルトレーニングは効率的に脂肪を燃やします。ハードだけど、挑戦する価値ありです！",
            "🏃 ストレッチも血行促進に効果的です。一緒に頑張りましょう。",
            "🏃 階段を使う、一駅歩くなど、日常で動く機会を増やしましょう。そばにいますよ。",
            "🏃 朝の運動は代謝を一日中高めてくれます。あなたならできます。",
            "🏃 筋トレ→有酸素運動の順番が脂肪燃焼に効果的です。信じていますよ。",
            "🏃 運動後のタンパク質摂取で筋肉の回復を。応援していますよ。",
            "🏃 無理のない範囲で、運動頻度を増やしてみましょう。大変だけど、やってみよう！",
            "🏃 ヨガやピラティスもインナーマッスルに効果的です。一緒に頑張りましょう。",
            "🏃 HIIT（高強度インターバルトレーニング）も検討してみては。きついけど効果抜群です！",
            "🏃 水泳は全身運動で関節にも優しいです。大変だけど、やってみよう！",
            "🏃 サイクリングも有酸素運動として優秀です。そばで見守っていますよ。",
            "🏃 スクワットは最強の筋トレです。毎日10回から始めましょう。きついけど、挑戦してみよう！",
            "🏃 プランクで体幹を鍛えると姿勢も良くなります。あなたならできます。",
            "🏃 運動は毎日じゃなくても大丈夫。週3-4回で十分です。応援していますよ。",
            "🏃 運動前のストレッチで怪我予防を忘れずに。信じていますよ。",
            "🏃 好きな音楽を聴きながら運動すると楽しく続けられます。一緒に頑張りましょう。",
            "🏃 運動仲間を作ると継続しやすくなります。そばにいますよ。",
            "🏃 散歩から始めるだけでも違います。小さな一歩から。いつも見守っていますよ。",
            "🏃 運動後は必ずクールダウンを。応援していますよ。",
            "🏃 体を動かすことでストレス解消にもなります。一緒に頑張りましょう。",
            "🏃 継続が何より大切。無理のない範囲で続けましょう。そばにいますよ。",
        ],

        # 睡眠のアドバイス - 25パターン
        "sleep": [
            "😴 睡眠時間は確保できていますか？7時間は眠りたいところです。応援していますよ。",
            "😴 睡眠不足は食欲ホルモンを乱します。しっかり眠りましょう。一緒に頑張りましょう。",
            "😴 質の良い睡眠が代謝を正常に保ちます。そばで見守っていますよ。",
            "😴 寝る前のスマホは控えめに。ブルーライトは睡眠を妨げます。あなたならできます。",
            "😴 就寝時間を一定にすると体内リズムが整います。信じていますよ。",
            "😴 寝室の温度は18-20度が理想的です。いつも応援しています。",
            "😴 寝る2時間前には食事を済ませましょう。応援していますよ。",
            "😴 カフェインは午後3時以降は控えめに。一緒に頑張りましょう。",
            "😴 アルコールは睡眠の質を下げます。程々に。そばにいますよ。",
            "😴 朝日を浴びると体内時計がリセットされます。あなたならできます。",
            "😴 昼寝は20分以内にとどめましょう。信じていますよ。",
            "😴 寝る前の軽いストレッチでリラックスを。いつも見守っていますよ。",
            "😴 暗くて静かな環境で眠りましょう。応援していますよ。",
            "😴 マグネシウムは睡眠の質を高めます。一緒に頑張りましょう。",
            "😴 睡眠不足だと脂肪が燃えにくくなります。そばで見守っていますよ。",
            "😴 成長ホルモンは睡眠中に分泌されます。あなたならできます。",
            "😴 睡眠負債は週末の寝だめでは解消できません。毎日7時間を。信じていますよ。",
            "😴 寝具にもこだわってみては。快適な睡眠環境を。応援していますよ。",
            "😴 ラベンダーの香りはリラックス効果があります。一緒に頑張りましょう。",
            "😴 瞑想や深呼吸で心を落ち着けてから眠りましょう。そばにいますよ。",
            "😴 入浴は就寝1-2時間前がベストです。いつも応援しています。",
            "😴 睡眠アプリで睡眠の質をチェックするのも良いですね。見守っていますよ。",
            "😴 夜更かしは代謝を乱します。規則正しく。応援していますよ。",
            "😴 睡眠は体の修復時間。しっかり眠ることも努力です。一緒に頑張りましょう。",
            "😴 疲れているのに眠れない時は、運動不足かもしれません。そばにいますよ。",
        ],

        # 食事のアドバイス - 25パターン
        "diet": [
            "🍽️ PFCバランスを見直してみましょう。タンパク質は足りていますか？応援していますよ。",
            "🍽️ 炭水化物を少し減らしてみるのも一つの方法です。大変だけど、やってみよう！",
            "🍽️ 食物繊維をしっかり摂ると腸内環境が整います。一緒に頑張りましょう。",
            "🍽️ 野菜を先に食べる「ベジファースト」を試してみては。あなたならできます。",
            "🍽️ 良質な脂質（オメガ3）を意識して摂りましょう。そばにいますよ。",
            "🍽️ 加工食品を減らして、自然な食材を増やしてみましょう。大変だけど、やってみよう！",
            "🍽️ 塩分の摂り過ぎは浮腫みの原因に。控えめに。信じていますよ。",
            "🍽️ よく噛んで食べることで満腹感が得られます。応援していますよ。",
            "🍽️ 食事の時間を一定にすると体内リズムが整います。一緒に頑張りましょう。",
            "🍽️ 間食を見直してみましょう。何を食べていますか？そばで見守っていますよ。",
            "🍽️ タンパク質を毎食摂ることで筋肉の分解を防げます。大変だけど、やってみよう！",
            "🍽️ 糖質は敵ではありません。質と量を意識しましょう。あなたならできます。",
            "🍽️ 発酵食品で腸内環境を整えるのも効果的です。応援していますよ。",
            "🍽️ 夜遅い食事は避けましょう。就寝3時間前には済ませたいですね。大変だけど、やってみよう！",
            "🍽️ チートデイを設けるのも一つの方法です。週1回だけ。一緒に頑張りましょう。",
            "🍽️ 食事の記録を続けることで見えてくるものがあります。信じていますよ。",
            "🍽️ 水溶性食物繊維が豊富な食材を取り入れましょう。そばにいますよ。",
            "🍽️ GI値の低い食品を選ぶと血糖値が安定します。応援していますよ。",
            "🍽️ タンパク質：体重×1.5-2gが目安です。いつも見守っていますよ。",
            "🍽️ 脂質を極端に減らすのは逆効果。適度に摂りましょう。一緒に頑張りましょう。",
            "🍽️ 色とりどりの野菜で様々な栄養素を。あなたならできます。",
            "🍽️ 食事を抜くのはNG。代謝が落ちます。そばにいますよ。",
            "🍽️ プロテインシェイクを活用するのも手です。応援していますよ。",
            "🍽️ お酒は糖質とカロリーが高め。控えめに。一緒に頑張りましょう。",
            "🍽️ 外食が続いた後は、自炊で調整しましょう。見守っていますよ。",
        ],

        # マインドセットのアドバイス - 25パターン
        "mindset": [
            "🧠 体重計の数字だけに囚われないで。見た目の変化も大切です。応援していますよ。",
            "🧠 完璧を求めすぎないこと。80点で十分です。一緒に頑張りましょう。",
            "🧠 停滞期は成長痛のようなもの。乗り越えれば強くなれます。そばにいますよ。",
            "🧠 比べるのは他人ではなく、過去の自分です。あなたならできます。",
            "🧠 短期的な結果より、長期的な習慣を大切に。信じていますよ。",
            "🧠 失敗は成功の元。諦めなければ失敗ではありません。応援していますよ。",
            "🧠 今日できることを、今日やる。それだけで十分です。一緒に頑張りましょう。",
            "🧠 ストレスは食欲を増進させます。リラックスも大切に。そばで見守っていますよ。",
            "🧠 自分を責めないでください。優しく接しましょう。いつも見守っていますよ。",
            "🧠 小さな成功を認めて、自分を褒めてあげてください。応援していますよ。",
            "🧠 体は日々変化しています。焦らず観察を。一緒に頑張りましょう。",
            "🧠 ダイエットは競争ではありません。マイペースで。そばにいますよ。",
            "🧠 目標は達成するためではなく、方向を示すためのもの。あなたならできます。",
            "🧠 できないことより、できることに目を向けましょう。信じていますよ。",
            "🧠 継続できていること自体が素晴らしい成果です。応援していますよ。",
            "🧠 一喜一憂せず、長い目で見ましょう。一緒に頑張りましょう。",
            "🧠 ネガティブな感情も受け入れて。それも自分です。そばで見守っていますよ。",
            "🧠 周りの評価より、自分の気持ちを大切に。いつも応援しています。",
            "🧠 完璧な一日より、継続できる毎日を。そばにいますよ。",
            "🧠 変化には時間がかかります。焦らないで。あなたならできます。",
            "🧠 自分の体を信じましょう。必ず応えてくれます。信じていますよ。",
            "🧠 やめたくなったら、始めた理由を思い出して。応援していますよ。",
            "🧠 今日も継続できた。それだけで100点です。一緒に頑張りましょう。",
            "🧠 過去は変えられない。でも未来は作れます。そばで見守っていますよ。",
            "🧠 自分への投資だと思って、楽しみましょう。いつも応援していますよ。",
        ],

        # 締めくくり - 25パターン
        "conclusion": [
            "明日も一緒に頑張りましょう！大変だけど、やってみよう！",
            "継続は力なり。信じて進みましょう。",
            "あなたなら絶対に乗り越えられます。大変だけど、やってみよう！",
            "小さな一歩を積み重ねていきましょう。",
            "今日も記録をつけたこと、素晴らしいです。",
            "諦めなければ、必ず結果はついてきます。",
            "この調子で、コツコツと続けていきましょう。大変だけど、やってみよう！",
            "停滞期を抜ければ、また変化が訪れます。きついけど、挑戦してみよう！",
            "今日より明日、少しでも前に進めればOKです。",
            "あなたの努力は必ず報われます。",
            "一緒に乗り越えていきましょう。",
            "焦らず、着実に、前を向いて。",
            "できることから、一つずつ。大変だけど、やってみよう！",
            "今日もお疲れさまでした。明日も応援しています。",
            "変化は必ず訪れます。信じて続けましょう。きついけど、挑戦してみよう！",
            "あなたのペースで大丈夫です。",
            "ここまで来たんです。後戻りはしません。大変だけど、やってみよう！",
            "次の一歩を、一緒に踏み出しましょう。",
            "停滞は一時的です。必ず動き出します。大変だけど、やってみよう！",
            "今日も頑張りましたね。自分を褒めてあげてください。",
            "継続できているあなたは、もう勝者です。",
            "明日への希望を持って、今日はゆっくり休んでください。",
            "小さな変化の積み重ねが、大きな結果を生みます。",
            "あなたの頑張りを、私は見ています。",
            "一緒にゴールを目指しましょう。必ず辿り着けます。",
        ],
    }


def generate_stagnation_advice():
    """
    体重停滞・増加時のアドバイスをランダムに生成

    Returns:
        str: 生成されたアドバイスメッセージ
    """
    templates = get_stagnation_advice_templates()

    # 各カテゴリからランダムに1つずつ選択
    introduction = random.choice(templates["introduction"])
    hydration = random.choice(templates["hydration"])
    supplements = random.choice(templates["supplements"])

    # 運動、睡眠、食事、マインドセットから2つをランダムに選択
    additional_categories = random.sample(
        ["exercise", "diet", "sleep", "mindset"],
        2
    )
    additional_advice = [
        random.choice(templates[cat]) for cat in additional_categories
    ]

    conclusion = random.choice(templates["conclusion"])

    # メッセージを組み立て
    message = f"\n💡 アドバイス:\n"
    message += f"{introduction}\n\n"
    message += f"{hydration}\n"
    message += f"{supplements}\n"
    for advice in additional_advice:
        message += f"{advice}\n"
    message += f"\n{conclusion}\n"

    return message


def generate_weekly_report(profile, weight_history, daily_records, week_number):
    """
    週刊レポート（メンターからの手紙）を生成

    Args:
        profile (dict): プロフィール情報
        weight_history (list): 体重履歴のリスト
        daily_records (list): 日々の記録データのリスト
        week_number (int): 週番号

    Returns:
        str: 週刊レポート
    """
    name = profile["name"]

    # 最近7日間のデータを抽出
    recent_days = min(7, len(daily_records))
    recent_records = daily_records[-recent_days:] if daily_records else []
    recent_weights = weight_history[-recent_days:] if len(weight_history) >= recent_days else weight_history

    # 体重変化を計算
    if len(recent_weights) >= 2:
        week_weight_change = recent_weights[-1] - recent_weights[0]
    else:
        week_weight_change = 0

    # カロリーの平均を計算
    calories_list = [r.get("calories") for r in recent_records if r.get("calories") is not None]
    avg_calories = sum(calories_list) / len(calories_list) if calories_list else None

    # 運動した日数をカウント
    exercise_days = sum(1 for r in recent_records
                       if r.get("exercise") and r["exercise"].lower() not in ["なし", "なし。", "特になし"])

    # レポート生成
    report = "\n" + "=" * 70 + "\n"
    report += "📬 メンターからの手紙 📬\n"
    report += f"【第{week_number}週 週刊レポート】\n"
    report += "=" * 70 + "\n\n"

    report += f"{name}さんへ\n\n"

    report += f"第{week_number}週、お疲れさまでした。\n"
    report += f"メンターとして、{name}さんのこの1週間の頑張りを振り返ります。\n\n"

    # 体重変化についてのコメント
    report += "【体重の変化】\n"
    if week_weight_change < -0.5:
        report += f"この1週間で{abs(week_weight_change):.1f}kg減少しました。\n"
        report += f"素晴らしい成果です！{name}さんの努力が数字に表れていますね。\n"
        report += f"この調子で、無理なく続けていきましょう。\n"
    elif week_weight_change > 0.5:
        report += f"この1週間で{week_weight_change:.1f}kg増加しました。\n"
        report += f"でも大丈夫。体重は日々変動するものです。\n"
        report += f"水分量やホルモンバランスでも簡単に1-2kg変わります。\n"
        report += f"焦らず、継続することが何より大切ですよ。\n"
    else:
        report += f"体重は概ね横ばいです（{week_weight_change:+.1f}kg）。\n"
        report += f"停滞期かもしれませんが、これは体が適応している証拠。\n"
        report += f"ここで諦めずに続けることで、次のステージに進めます。\n"
    report += "\n"

    # 運動についてのコメント
    report += "【運動の継続状況】\n"
    if exercise_days >= 5:
        report += f"この1週間で{exercise_days}日も運動されました。\n"
        report += f"素晴らしい継続力です！運動習慣がしっかり身についていますね。\n"
    elif exercise_days >= 3:
        report += f"この1週間で{exercise_days}日運動されました。\n"
        report += f"週3-4回の運動、理想的なペースです。この調子で続けましょう。\n"
    elif exercise_days >= 1:
        report += f"この1週間で{exercise_days}日運動されました。\n"
        report += f"運動できた日があること、それ自体が素晴らしいです。\n"
        report += f"できる範囲で少しずつ増やしていきましょう。\n"
    else:
        report += f"この1週間は運動の記録がありませんでした。\n"
        report += f"大丈夫です。まずは5分のウォーキングから始めてみませんか？\n"
        report += f"小さな一歩が、大きな変化を生みます。\n"
    report += "\n"

    # 食事についてのコメント
    if avg_calories:
        target_cal = profile.get("target_calories", 0)
        cal_diff = avg_calories - target_cal
        cal_diff_pct = (cal_diff / target_cal * 100) if target_cal > 0 else 0

        report += "【食事の状況】\n"
        report += f"この1週間の平均摂取カロリーは{avg_calories:.0f}kcalでした。\n"
        report += f"（目標: {target_cal:.0f}kcal）\n\n"

        if abs(cal_diff_pct) < 10:
            report += f"目標カロリーに対して±10%以内、完璧です！\n"
            report += f"この精度で続けられれば、確実に結果がついてきます。\n"
        elif cal_diff > 0:
            report += f"目標より少しオーバー気味ですが、大丈夫です。\n"
            report += f"数字はあくまで目安。完璧を求めすぎず、80点を目指しましょう。\n"
        else:
            report += f"目標より少なめですね。\n"
            report += f"極端に減らしすぎると代謝が落ちるので、\n"
            report += f"しっかり食べることも大切ですよ。\n"
        report += "\n"

    # 励ましのメッセージ
    report += "【メンターからのメッセージ】\n\n"

    encouragement_messages = [
        f"{name}さん、この1週間も本当によく頑張りましたね。\n",
        f"{name}さんの継続力、素晴らしいです。\n",
        f"{name}さん、着実に前進していますよ。\n",
        f"{name}さん、この努力は必ず報われます。\n",
    ]
    report += random.choice(encouragement_messages)

    report += "\n"
    report += "ダイエットは一直線には進みません。\n"
    report += "良い日もあれば、思うようにいかない日もあります。\n"
    report += "でも、それが普通なんです。\n\n"

    report += "大切なのは、完璧を目指すことではなく、\n"
    report += "諦めずに続けることです。\n\n"

    advice_messages = [
        "来週も、今週と同じように続けていきましょう。\n",
        "来週は、今週より少しだけ意識を高めてみませんか。\n",
        "来週も、あなたのペースで大丈夫です。\n",
        "来週も、一緒に頑張っていきましょう。\n",
    ]
    report += random.choice(advice_messages)

    report += "\n"
    report += f"いつも{name}さんのことを応援しています。\n"
    report += "一緒に、目標に向かって進んでいきましょう！\n\n"

    report += "💪 ファイト！\n\n"
    report += "― ダイエットメンターより\n"
    report += "=" * 70 + "\n"

    return report


def generate_monthly_report(profile, weight_history, daily_records, month_number):
    """
    月間レポート（メンターからの手紙）を生成

    Args:
        profile (dict): プロフィール情報
        weight_history (list): 体重履歴のリスト
        daily_records (list): 日々の記録データのリスト
        month_number (int): 月番号

    Returns:
        str: 月間レポート
    """
    name = profile["name"]

    # 最近30日間のデータを抽出
    recent_days = min(30, len(daily_records))
    recent_records = daily_records[-recent_days:] if daily_records else []
    recent_weights = weight_history[-recent_days:] if len(weight_history) >= recent_days else weight_history

    # 体重変化を計算
    if len(recent_weights) >= 2:
        month_weight_change = recent_weights[-1] - recent_weights[0]
        initial_weight = recent_weights[0]
        current_weight = recent_weights[-1]
        change_percentage = (month_weight_change / initial_weight * 100) if initial_weight > 0 else 0
    else:
        month_weight_change = 0
        current_weight = profile.get("current_weight", 0)
        change_percentage = 0

    # カロリーの平均を計算
    calories_list = [r.get("calories") for r in recent_records if r.get("calories") is not None]
    avg_calories = sum(calories_list) / len(calories_list) if calories_list else None

    # PFCの平均を計算
    protein_list = [r.get("protein") for r in recent_records if r.get("protein") is not None]
    fat_list = [r.get("fat") for r in recent_records if r.get("fat") is not None]
    carbs_list = [r.get("carbs") for r in recent_records if r.get("carbs") is not None]

    avg_protein = sum(protein_list) / len(protein_list) if protein_list else None
    avg_fat = sum(fat_list) / len(fat_list) if fat_list else None
    avg_carbs = sum(carbs_list) / len(carbs_list) if carbs_list else None

    # 運動した日数をカウント
    exercise_days = sum(1 for r in recent_records
                       if r.get("exercise") and r["exercise"].lower() not in ["なし", "なし。", "特になし"])
    exercise_rate = (exercise_days / recent_days * 100) if recent_days > 0 else 0

    # 体重が減少した日数をカウント
    decrease_days = 0
    for i in range(1, len(recent_weights)):
        if recent_weights[i] < recent_weights[i-1]:
            decrease_days += 1

    # レポート生成
    report = "\n" + "=" * 70 + "\n"
    report += "📬 メンターからの手紙 📬\n"
    report += f"【第{month_number}ヶ月 月間レポート】\n"
    report += "=" * 70 + "\n\n"

    report += f"{name}さんへ\n\n"

    report += f"早いもので、もう{month_number}ヶ月が経ちました。\n"
    report += f"メンターとして、{name}さんのこの1ヶ月間の成長を\n"
    report += f"じっくり振り返りたいと思います。\n\n"

    # 体重変化についての詳細なコメント
    report += "【この1ヶ月の体重変化】\n"
    report += f"開始時: {recent_weights[0]:.1f}kg → 現在: {current_weight:.1f}kg\n"
    report += f"変化: {month_weight_change:+.1f}kg（{change_percentage:+.1f}%）\n\n"

    if month_weight_change < -1:
        report += f"この1ヶ月で{abs(month_weight_change):.1f}kg減少しました。\n"
        report += f"本当に素晴らしい成果です！\n\n"

        if abs(change_percentage) >= 2 and abs(change_percentage) <= 4:
            report += f"月に体重の{abs(change_percentage):.1f}%減、理想的なペースです。\n"
            report += f"この範囲であれば、リバウンドのリスクも低く、\n"
            report += f"健康的に減量できています。\n"
        elif abs(change_percentage) < 2:
            report += f"ゆっくりとしたペースですが、それが一番確実です。\n"
            report += f"急激な減量よりも、着実な減量の方が\n"
            report += f"長期的には成功しやすいんですよ。\n"
        else:
            report += f"少しペースが速いかもしれません。\n"
            report += f"素晴らしい結果ですが、体への負担も考えて、\n"
            report += f"無理をしすぎないように気をつけましょう。\n"
    elif month_weight_change > 1:
        report += f"この1ヶ月で{month_weight_change:.1f}kg増加しました。\n"
        report += f"でも、落ち込む必要は全くありません。\n\n"
        report += f"体重は本当に様々な要因で変動します。\n"
        report += f"水分、筋肉量、ホルモンバランス、ストレス...\n"
        report += f"数字だけで判断するのは早計です。\n\n"
        report += f"大切なのは、この1ヶ月間、{name}さんが\n"
        report += f"諦めずに続けてきたという事実です。\n"
        report += f"それこそが最大の成果なんですよ。\n"
    else:
        report += f"体重はほぼ横ばいです。\n"
        report += f"これは停滞期の可能性が高いですね。\n\n"
        report += f"停滞期は誰にでも訪れます。\n"
        report += f"体が新しい状態に適応しようとしている証拠です。\n"
        report += f"ここで諦めずに続けることが、\n"
        report += f"次のブレイクスルーにつながります。\n"
    report += "\n"

    # 運動についての詳細なコメント
    report += "【運動の継続状況】\n"
    report += f"運動した日数: {exercise_days}日 / {recent_days}日（{exercise_rate:.0f}%）\n\n"

    if exercise_rate >= 70:
        report += f"素晴らしい継続率です！\n"
        report += f"運動習慣が完全に定着していますね。\n"
        report += f"この習慣こそが、{name}さんの一生の財産になります。\n"
    elif exercise_rate >= 40:
        report += f"良い継続率です。\n"
        report += f"週3-4回のペースで運動できているということですね。\n"
        report += f"このペースなら無理なく続けられます。\n"
    elif exercise_rate >= 20:
        report += f"運動の習慣づくりの途中ですね。\n"
        report += f"焦らなくて大丈夫です。\n"
        report += f"来月は、今月よりほんの少しだけ回数を増やすことを\n"
        report += f"目標にしてみませんか？\n"
    else:
        report += f"運動の習慣がまだ定着していないようですね。\n"
        report += f"でも大丈夫。最初はみんなそうです。\n"
        report += f"まずは週1回、5分のウォーキングから。\n"
        report += f"小さな一歩が、大きな変化を生みます。\n"
    report += "\n"

    # 食事についての詳細なコメント
    if avg_calories:
        target_cal = profile.get("target_calories", 0)

        report += "【食事の状況】\n"
        report += f"平均摂取カロリー: {avg_calories:.0f}kcal/日\n"
        report += f"目標カロリー: {target_cal:.0f}kcal/日\n"

        if avg_protein and avg_fat and avg_carbs:
            report += f"\n平均PFCバランス:\n"
            report += f"  タンパク質(P): {avg_protein:.1f}g\n"
            report += f"  脂質(F): {avg_fat:.1f}g\n"
            report += f"  炭水化物(C): {avg_carbs:.1f}g\n"

        report += "\n"

        cal_diff = avg_calories - target_cal
        cal_diff_pct = (cal_diff / target_cal * 100) if target_cal > 0 else 0

        if abs(cal_diff_pct) < 10:
            report += f"素晴らしい！目標に対して±10%以内です。\n"
            report += f"食事管理がしっかりできていますね。\n"
        elif cal_diff > 0:
            report += f"目標より{cal_diff:.0f}kcal多めですが、\n"
            report += f"完璧である必要はありません。\n"
            report += f"大切なのは、意識し続けることです。\n"
        else:
            report += f"目標より{abs(cal_diff):.0f}kcal少なめです。\n"
            report += f"減らしすぎは代謝の低下につながるので、\n"
            report += f"もう少し食べても大丈夫ですよ。\n"
        report += "\n"

    # データ分析
    report += "【この1ヶ月の分析】\n"
    if decrease_days > 0:
        report += f"体重が減少した日: {decrease_days}日\n"
        report += f"着実に前進している証拠です。\n"
    report += "\n"

    # 励ましと次月への展望
    report += "【メンターからのメッセージ】\n\n"

    report += f"{name}さん、この1ヶ月間、本当にお疲れさまでした。\n\n"

    report += "数字に表れる成果も大切ですが、\n"
    report += "それ以上に大切なのは、この1ヶ月間、\n"
    report += f"{name}さんが諦めずに続けてきたという事実です。\n\n"

    report += "継続は、誰にでもできそうで、実は一番難しいこと。\n"
    report += f"それを成し遂げている{name}さんは、\n"
    report += "もう既に大きな成功を収めているんです。\n\n"

    report += "来月も、今月と同じように、\n"
    report += "あなたのペースで進んでいきましょう。\n\n"

    report += "完璧を目指す必要はありません。\n"
    report += "80点で十分。80点を継続することが、\n"
    report += "100点を一度取るよりも遥かに価値があります。\n\n"

    report += f"{name}さんなら、必ず目標に辿り着けます。\n"
    report += "私はそれを確信しています。\n\n"

    report += "来月も、一緒に頑張りましょう！\n"
    report += f"いつも{name}さんのことを応援しています。\n\n"

    report += "💪 ファイト！\n\n"
    report += "― ダイエットメンターより\n"
    report += "=" * 70 + "\n"

    return report


def save_report_to_file(report, name, report_type, period_number):
    """
    レポートをファイルに保存

    Args:
        report (str): レポート内容
        name (str): ユーザー名
        report_type (str): "weekly" or "monthly"
        period_number (int): 週番号または月番号

    Returns:
        str: 保存したファイルパス
    """
    import datetime

    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')

    if report_type == "weekly":
        filename = f"weekly_report_{name}_week{period_number}_{timestamp}.txt"
    else:
        filename = f"monthly_report_{name}_month{period_number}_{timestamp}.txt"

    filepath = os.path.join(os.getcwd(), filename)

    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(report)
        return filepath
    except Exception as e:
        print(f"⚠️ レポートの保存中にエラーが発生しました: {e}")
        return None


def generate_graduation_message(profile, weight_history):
    """
    卒業時の紆余曲折を振り返るメッセージを生成

    Args:
        profile (dict): プロフィール情報
        weight_history (list): 体重履歴のリスト（日付とペアではなく体重のみのリスト）

    Returns:
        str: 卒業メッセージ
    """
    name = profile["name"]
    initial_weight = profile["initial_weight"]
    final_weight = weight_history[-1] if weight_history else initial_weight
    total_loss = initial_weight - final_weight
    duration = len(weight_history)

    # 体重変化の分析
    max_weight = max(weight_history) if weight_history else initial_weight
    min_weight = min(weight_history) if weight_history else initial_weight

    # 増加した日数をカウント
    increase_days = 0
    decrease_days = 0
    for i in range(1, len(weight_history)):
        if weight_history[i] > weight_history[i-1]:
            increase_days += 1
        elif weight_history[i] < weight_history[i-1]:
            decrease_days += 1

    # メッセージ生成
    message = f"\n{'='*70}\n"
    message += f"【{name}さんの{duration}日間の軌跡】\n"
    message += f"{'='*70}\n\n"

    message += f"{name}さん、{duration}日間、本当にお疲れさまでした。\n\n"

    # ドラマを振り返る
    message += "この期間を振り返ると...\n\n"

    if increase_days > 0:
        message += f"体重が増えた日が{increase_days}日ありました。\n"
        message += f"その時、きっと悔しかったでしょう。でも諦めなかった。\n"
        message += f"それが今日の{name}さんを作ったんです。\n\n"

    if decrease_days > 0:
        message += f"体重が減った日は{decrease_days}日。\n"
        message += f"一歩ずつ、着実に前進した証です。\n\n"

    if max_weight > initial_weight:
        message += f"最高体重{max_weight:.1f}kgを記録した日もありました。\n"
        message += f"でも、そこから{max_weight - final_weight:.1f}kg減らしたんです。\n"
        message += f"これは紛れもなく{name}さんの努力の結果です。\n\n"

    message += f"最終的に{total_loss:.1f}kg減という成果。\n"
    message += f"この数字は、{name}さんが歩んできた道のりを物語っています。\n\n"

    # 励ましの言葉
    message += "💭 今日で終わりではありません。\n"
    message += "   ここからが本当のスタートです。\n\n"

    message += f"{name}さんなら、きっと大丈夫。\n"
    message += f"この{duration}日間で培った習慣と自信を胸に、\n"
    message += f"これからも{name}さんらしく、前を向いて進んでください。\n\n"

    message += "いつでも応援しています！💪\n\n"
    message += f"― ダイエットメンターより、{name}さんへ\n"
    message += f"{'='*70}\n"

    return message


def print_comparison_table(actual_calories, target_calories, actual_protein, target_protein,
                           actual_fat, target_fat, actual_carbs, target_carbs):
    """
    カロリーとPFCの比較表を表示

    Args:
        actual_calories (float): 実際のカロリー
        target_calories (float): 目標カロリー
        actual_protein (float): 実際のタンパク質
        target_protein (float): 目標タンパク質
        actual_fat (float): 実際の脂質
        target_fat (float): 目標脂質
        actual_carbs (float): 実際の炭水化物
        target_carbs (float): 目標炭水化物
    """
    print("\n" + "=" * 70)
    print("【栄養素比較表】")
    print("=" * 70)
    print(f"{'項目':<12} {'実際':<15} {'目標':<15} {'差異':<15}")
    print("-" * 70)

    # カロリー
    cal_diff = actual_calories - target_calories
    cal_diff_pct = (cal_diff / target_calories * 100) if target_calories > 0 else 0
    print(f"{'カロリー':<10} {actual_calories:>8.0f}kcal   {target_calories:>8.0f}kcal   {cal_diff:>+8.0f}kcal ({cal_diff_pct:>+5.1f}%)")

    # タンパク質
    if actual_protein is not None and target_protein > 0:
        p_diff = actual_protein - target_protein
        p_diff_pct = (p_diff / target_protein * 100)
        print(f"{'タンパク質':<8} {actual_protein:>8.1f}g      {target_protein:>8.1f}g      {p_diff:>+8.1f}g   ({p_diff_pct:>+5.1f}%)")

    # 脂質
    if actual_fat is not None and target_fat > 0:
        f_diff = actual_fat - target_fat
        f_diff_pct = (f_diff / target_fat * 100)
        print(f"{'脂質':<12} {actual_fat:>8.1f}g      {target_fat:>8.1f}g      {f_diff:>+8.1f}g   ({f_diff_pct:>+5.1f}%)")

    # 炭水化物
    if actual_carbs is not None and target_carbs > 0:
        c_diff = actual_carbs - target_carbs
        c_diff_pct = (c_diff / target_carbs * 100)
        print(f"{'炭水化物':<10} {actual_carbs:>8.1f}g      {target_carbs:>8.1f}g      {c_diff:>+8.1f}g   ({c_diff_pct:>+5.1f}%)")

    print("=" * 70)
    print()


def calculate_target_calories_and_pfc(profile):
    """
    目標カロリーとPFCバランスを計算

    Args:
        profile (dict): プロフィール情報

    Returns:
        dict: 目標カロリー、P/F/C（g）
    """
    # 基礎代謝量を計算
    bmr = calculate_bmr(
        profile["gender"],
        profile["current_weight"],
        profile["height"],
        profile["age"]
    )

    # 総消費カロリー（TDEE）を計算
    tdee = calculate_tdee(bmr, profile["activity_coefficient"])

    # ダイエットモードに応じたカロリー調整
    # 月に体重の2%（ライトモード）または4%（ハードモード）を減らすための1日あたりのカロリー不足を計算
    # 脂肪1kg = 約7200kcal
    weight_loss_per_month = profile["current_weight"] * profile["reduction_rate"]
    weight_loss_per_day = weight_loss_per_month / 30
    calorie_deficit_per_day = weight_loss_per_day * 7200

    # 目標カロリー = TDEE - カロリー不足
    target_calories = tdee - calorie_deficit_per_day

    # PFCバランスを計算（P:30%, F:20%, C:50%）
    protein_calories = target_calories * 0.30
    fat_calories = target_calories * 0.20
    carbs_calories = target_calories * 0.50

    # カロリーをグラムに変換（P:4kcal/g, F:9kcal/g, C:4kcal/g）
    protein_grams = protein_calories / 4
    fat_grams = fat_calories / 9
    carbs_grams = carbs_calories / 4

    return {
        "bmr": bmr,
        "tdee": tdee,
        "target_calories": target_calories,
        "protein": protein_grams,
        "fat": fat_grams,
        "carbs": carbs_grams
    }


def select_plan():
    """
    プランを選択する（現在は990円プランのみ）

    Returns:
        dict: プラン情報（プラン名、料金）
    """
    print("【プラン料金】")
    print("=" * 60)
    print("💰 990円プラン")
    print("   - ライトモードまたはハードモードをお選びいただけます")
    print("   - いつでも解約可能")
    print("=" * 60)
    print()


    return {
        "plan_name": "990円プラン",
        "price": "990円"
    }


def get_profile():
    """
    ユーザーのプロフィール情報を取得する

    Returns:
        dict: プロフィール情報（名前、ダイエットモード、期間、現在体重、目標体重）
    """
    print("【プロフィール設定】")
    print("まずは、あなたのことを教えてください！\n")

    # 名前の入力

    print(f"\n{name}さん、よろしくお願いします！")

    # 性別の入力
    print("\n【性別】性別を教えてください")
    while True:
        if gender == "1":
            gender = "男性"
            break
        elif gender == "2":
            gender = "女性"
            break
        else:
            print("⚠️ 1 または 2 を入力してください")

    # 年齢の入力
    print("\n【年齢】年齢を教えてください")
    while True:
        try:
            if age > 0 and age < 120:
                break
            else:
                print("⚠️ 正しい年齢を入力してください")
        except ValueError:
            print("⚠️ 数字を入力してください")

    # 身長の入力
    print("\n【身長】身長は何cmですか？")
    while True:
        try:
            if height > 0 and height < 250:
                break
            else:
                print("⚠️ 正しい身長を入力してください")
        except ValueError:
            print("⚠️ 数字を入力してください")

    # 活動レベルの入力
    print("\n【活動レベル】普段の活動レベルを選択してください")
    print("=" * 60)
    print("1. 低い：座り仕事中心（係数1.2）")
    print("2. やや低い：日常生活+週1回程度運動（係数1.375）")
    print("3. 中程度：週3回トレーニング（係数1.55）")
    print("4. 高い：週4~5回トレーニング（係数1.725）")
    print("5. 非常に高い：肉体労働+週5回以上トレーニング（係数1.9）")
    print("=" * 60)

    activity_levels = {
        "1": ("低い", 1.2),
        "2": ("やや低い", 1.375),
        "3": ("中程度", 1.55),
        "4": ("高い", 1.725),
        "5": ("非常に高い", 1.9)
    }

    while True:
        if activity_choice in activity_levels:
            activity_name, activity_coefficient = activity_levels[activity_choice]
            break
        else:
            print("⚠️ 1~5 のいずれかを入力してください")

    # ダイエットモードの選択
    print("\n【ダイエットモード】どのペースで進めますか？")
    print("=" * 60)
    print("1. ライトモード")
    print("   - 月に体重の2%ペースで減量")
    print("   - 無理なく続けたい方におすすめ")
    print("   - リバウンドしにくい健康的なペース")
    print()
    print("2. ハードモード")
    print("   - 月に体重の4%ペースで減量")
    print("   - しっかり結果を出したい方におすすめ")
    print("   - より計画的な食事管理が必要")
    print("=" * 60)

    while True:
        if mode_choice == "1":
            diet_mode = "ライトモード"
            reduction_rate = 0.02  # 2%
            break
        elif mode_choice == "2":
            diet_mode = "ハードモード"
            reduction_rate = 0.04  # 4%
            break
        else:
            print("⚠️ 1 または 2 を入力してください")

    # 現在の体重の入力
    print("\n【現在の体重】現在の体重は何kgですか？")
    while True:
        try:
            if current_weight > 0:
                break
            else:
                print("⚠️ 正しい体重を入力してください")
        except ValueError:
            print("⚠️ 数字を入力してください")

    # 目標体重の入力
    print("\n【目標体重】目標体重は何kgですか？")
    while True:
        try:
            if target_weight > 0 and target_weight < current_weight:
                break
            elif target_weight >= current_weight:
                print("⚠️ 目標体重は現在の体重より軽くしてください（ダイエット専用です）")
            else:
                print("⚠️ 正しい体重を入力してください")
        except ValueError:
            print("⚠️ 数字を入力してください")

    # 関口からの提案
    monthly_loss = current_weight * reduction_rate
    print("\n" + "=" * 60)
    print("【関口からの提案】")
    print("=" * 60)
    print(f"\n{name}さん、{diet_mode}を選択されましたね。")
    print(f"現在の体重{current_weight}kgから、無理なく健康的に減量するなら...")
    print()
    print(f"💡 月に{monthly_loss:.1f}kgずつ")
    print(f"   落としていくペースがおすすめです！")
    print()
    print(f"目標体重の{target_weight}kgまで、一緒に頑張りましょう！")
    print("=" * 60)
    print()


    # プロフィール情報をまとめる
    profile = {
        "name": name,
        "gender": gender,
        "age": age,
        "height": height,
        "activity_name": activity_name,
        "activity_coefficient": activity_coefficient,
        "diet_mode": diet_mode,
        "reduction_rate": reduction_rate,
        "current_weight": current_weight,
        "target_weight": target_weight,
        "initial_weight": current_weight  # 初期体重を記録
    }

    return profile


def show_profile_summary(profile):
    """
    入力されたプロフィールの確認画面を表示

    Args:
        profile (dict): プロフィール情報
    """
    # 目標カロリーとPFCを計算
    targets = calculate_target_calories_and_pfc(profile)

    # プロフィールに目標値を追加
    profile["target_calories"] = targets["target_calories"]
    profile["target_protein"] = targets["protein"]
    profile["target_fat"] = targets["fat"]
    profile["target_carbs"] = targets["carbs"]
    profile["bmr"] = targets["bmr"]
    profile["tdee"] = targets["tdee"]

    print("\n" + "=" * 60)
    print("【プロフィール確認】")
    print("=" * 60)
    print(f"お名前: {profile['name']}さん")
    print(f"性別: {profile['gender']} / 年齢: {profile['age']}歳 / 身長: {profile['height']}cm")
    print(f"活動レベル: {profile['activity_name']}（係数{profile['activity_coefficient']}）")
    print(f"ダイエットモード: {profile['diet_mode']}（月{int(profile['reduction_rate']*100)}%ペース）")
    print(f"プラン: {profile['plan_name']} ({profile['price']})")
    print()
    print(f"現在の体重: {profile['current_weight']}kg")
    print(f"目標体重: {profile['target_weight']}kg")

    # 減量目標の計算
    weight_diff = profile['current_weight'] - profile['target_weight']
    print(f"目標: {weight_diff:.1f}kg減量")
    print()

    print("【科学的な目標設定】")
    print("-" * 60)
    print(f"基礎代謝量（BMR）: {targets['bmr']:.0f}kcal/日")
    print(f"総消費カロリー（TDEE）: {targets['tdee']:.0f}kcal/日")
    print()
    print(f"📊 1日の目標摂取カロリー: {targets['target_calories']:.0f}kcal")
    print(f"💪 タンパク質（P）: {targets['protein']:.1f}g（30%）")
    print(f"🥑 脂質（F）: {targets['fat']:.1f}g（20%）")
    print(f"🍚 炭水化物（C）: {targets['carbs']:.1f}g（50%）")
    print()
    print("💡 これらの数字はあくまで目安です。")
    print("   ±10%の範囲なら全く問題ありません。")
    print("   大切なのは完璧を目指すことではなく、続けることです。")

    print("=" * 60)
    print()

    # メニュー提案を表示
    suggest_meal_examples(
        targets['target_calories'],
        targets['protein'],
        targets['fat'],
        targets['carbs'],
        profile['name']
    )



    """
    今日の報告を入力してもらう

    Args:
        profile (dict): プロフィール情報

    Returns:
        dict or None: 今日の報告データ（終了の場合はNone）
    """
    print("\n" + "-" * 60)
    print("【今日の報告】")
    print("-" * 60)

    # 体重の入力（終了判定も含む）
    print("【体重】今日の体重は何kgですか？")
    print("（終了する場合は「exit」または「終了」と入力してください）")

    # 終了判定


    # 体重を数値に変換
    weight = profile.get("current_weight", profile.get("initial_weight", 0))

    # 運動の入力
    print("\n【運動】今日やった運動を教えてください")
    print("（例: 30分ジョギング、筋トレ、なし など）")

    # 食事の入力方法を選択
    print("\n【食事】食事の入力方法を選択してください")
    print("1. 画像をアップロードして自動計算")
    print("2. 手動で入力")

    meal = None
    calories = None
    protein = None
    fat = None
    carbs = None

    while True:

        if choice == "1":
            # 画像アップロード
            image_path = get_image_path_from_user()

            if image_path:
                # APIキーを環境変数から取得（初回のみ設定を促す）
                api_key = os.environ.get("GEMINI_API_KEY")
                if not api_key:
                    print("\n【初回設定】Google Gemini APIキーを入力してください")
                    print("（取得方法: https://makersuite.google.com/app/apikey）")
                    # 環境変数に設定（このセッションのみ有効）
                    os.environ["GEMINI_API_KEY"] = api_key

                # 画像を解析
                analysis_result = analyze_food_image(image_path, api_key)

                if analysis_result:
                    meal = analysis_result["meal_description"]
                    calories = analysis_result["total_calories"]
                    protein = analysis_result["protein"]
                    fat = analysis_result["fat"]
                    carbs = analysis_result["carbs"]

                    # 結果を表示
                    print("\n✅ 解析完了！")
                    print(f"📝 食事: {meal}")
                    print(f"🔥 カロリー: {calories}kcal")
                    print(f"💪 P: {protein}g / 🥑 F: {fat}g / 🍚 C: {carbs}g")
                    break
                else:
                    print("⚠️ 解析に失敗しました。手動入力に切り替えますか？（y/n）")
                    if retry == "y":
                        choice = "2"
                        continue
            else:
                # 画像パス取得をキャンセルした場合
                print("手動入力に切り替えます")
                choice = "2"
                continue

        elif choice == "2":
            # 手動入力
            print("\n【食事】今日食べたものをざっくり教えてください")
            print("（例: 鶏胸肉、ブロッコリー、白米 など）")

            # カロリーの入力
            print("\n【カロリー】今日の総カロリーは何kcalですか？")
            print("（不明な場合は「不明」と入力してください）")
            if cal_#input.lower() not in ["不明", "わからない", ""]:
                try:
                    calories = float(cal_#input)
                except ValueError:
                    print("⚠️ 正しい数字を入力してください")
                    continue

            # PFCの入力
            print("\n【PFC】タンパク質(P)、脂質(F)、炭水化物(C)を入力しますか？（y/n）")

            if pfc_choice == "y":
                # タンパク質
                print("タンパク質(P)は何gですか？")
                try:
                except ValueError:
                    protein = None

                # 脂質
                print("脂質(F)は何gですか？")
                try:
                except ValueError:
                    fat = None

                # 炭水化物
                print("炭水化物(C)は何gですか？")
                try:
                except ValueError:
                    carbs = None

            break
        else:
            print("⚠️ 1 または 2 を入力してください")

    # 今日のデータをまとめる
    daily_data = {
        "weight": weight,
        "exercise": exercise,
        "meal": meal,
        "calories": calories,
        "protein": protein,
        "fat": fat,
        "carbs": carbs
    }

    return daily_data


def generate_feedback(profile, daily_data):
    """
    関口さん風のフィードバックを生成する

    Args:
        profile (dict): プロフィール情報
        daily_data (dict): 今日の報告データ
    """
    name = profile["name"]
    diet_mode = profile["diet_mode"]
    current_weight = daily_data["weight"]
    initial_weight = profile["initial_weight"]
    target_weight = profile["target_weight"]
    exercise = daily_data["exercise"]
    meal = daily_data["meal"]
    calories = daily_data.get("calories")
    protein = daily_data.get("protein")
    fat = daily_data.get("fat")
    carbs = daily_data.get("carbs")

    # 体重変化を計算
    weight_change = current_weight - initial_weight
    weight_to_goal = current_weight - target_weight  # 減量なので現在-目標

    print("\n" + "=" * 60)
    print("【ダイエットメンターからのフィードバック】")
    print("=" * 60)
    print()

    # 栄養情報を表示（データがある場合）
    if calories is not None or any([protein, fat, carbs]):
        # 目標値を取得
        target_cal = profile.get("target_calories", 0)
        target_p = profile.get("target_protein", 0)
        target_f = profile.get("target_fat", 0)
        target_c = profile.get("target_carbs", 0)

        # カロリーのみ、または全てのデータがある場合に比較表とグラフを表示
        if calories is not None:
            # 比較表を表示
            print_comparison_table(
                calories, target_cal,
                protein if protein is not None else 0,
                target_p,
                fat if fat is not None else 0,
                target_f,
                carbs if carbs is not None else 0,
                target_c
            )

            # グラフを生成して保存
            if protein is not None and fat is not None and carbs is not None:
                graph_path = create_nutrition_graphs(
                    name, calories, target_cal,
                    protein, target_p,
                    fat, target_f,
                    carbs, target_c
                )
                print(f"📊 栄養比較グラフを保存しました: {graph_path}")
                print()

        # カロリーの簡易表示と評価（柔軟性を持たせたメッセージ）
        if calories is not None:
            cal_diff = calories - target_cal
            cal_diff_pct = (cal_diff / target_cal * 100) if target_cal > 0 else 0

            print("📊 今日の評価:")
            if abs(cal_diff_pct) < 5:
                print(f"  🎯 カロリー: 目標ピッタリです！素晴らしい👍")
            elif abs(cal_diff_pct) < 10:
                # ±10%以内なら許容範囲
                print(f"  ✅ カロリー: {calories:.0f}kcal（目標{target_cal:.0f}kcal）")
                print(f"     → ±10%以内なら全く問題ありません。大事なのは続けることです。")
            elif cal_diff > 0:
                # 10%以上オーバー
                print(f"  ⚠️ カロリー: 目標より{abs(cal_diff):.0f}kcal多いです（{cal_diff_pct:.1f}%オーバー）")
                print(f"     → 数字はあくまで目安。明日から調整していきましょう。")
            else:
                # 10%以上少ない
                print(f"  ✅ カロリー: 目標より{abs(cal_diff):.0f}kcal少ないです")
                print(f"     → 極端に減らしすぎると代謝が落ちます。適度に食べることも大切ですよ。")

            # PFCバランスを計算（全て入力されている場合）
            if protein and fat and carbs:
                total_macros = protein * 4 + fat * 9 + carbs * 4  # カロリーに換算
                p_ratio = (protein * 4 / total_macros) * 100 if total_macros > 0 else 0
                f_ratio = (fat * 9 / total_macros) * 100 if total_macros > 0 else 0
                c_ratio = (carbs * 4 / total_macros) * 100 if total_macros > 0 else 0

                print(f"  📈 PFCバランス: P{p_ratio:.0f}% / F{f_ratio:.0f}% / C{c_ratio:.0f}%")
                print(f"  🎯 理想バランス: P30% / F20% / C50%")
                print(f"     → あくまで目安です。人によって合う比率は違いますからね。")

        print()

    # 体重変化のコメント
    print(f"📊 体重変化: ", end="")
    if weight_change < 0:
        print(f"{abs(weight_change):.1f}kg減 👍")
        print(f"素晴らしい！目標まであと{weight_to_goal:.1f}kgです。")
    elif weight_change > 0:
        print(f"{weight_change:.1f}kg増")
        print(f"焦らなくて大丈夫！体重は日々変動します。")
        # ランダムなアドバイスを表示
        advice = generate_stagnation_advice()
        print(advice)
    else:
        print(f"変化なし")
        print(f"体重は日々変動するものです。継続が大切ですよ！")
        # ランダムなアドバイスを表示
        advice = generate_stagnation_advice()
        print(advice)

    print()
    print(f"🎯 {name}さんへ: {diet_mode}ダイエット、順調に進んでいますよ！")
    print()

    # 良かった点をランダムに生成
    print("✨ 良かった点:")
    good_points = []

    # 基本的な良かった点
    good_points.append(f"  • {name}さん、今日も記録をつけてくれてありがとう！これが継続の第一歩です")

    # 運動について
    if exercise and exercise.lower() not in ["なし", "なし。", "特になし"]:
        exercise_compliments = [
            f"  • 「{exercise}」をやったこと、素晴らしいです！",
            f"  • 今日も運動できたんですね。その積み重ねが結果につながります",
            f"  • 「{exercise}」、よく頑張りましたね！",
        ]
        good_points.append(random.choice(exercise_compliments))

    # 食事について
    if meal:
        meal_compliments = [
            f"  • 食事の内容を意識していること、それだけで大きな一歩です",
            f"  • しっかり食べていますね。無理な食事制限より、バランスが大事ですよ",
            f"  • 食事を記録する習慣、これがとても大切です！",
        ]
        good_points.append(random.choice(meal_compliments))

    # 体重測定について
    measurement_compliments = [
        f"  • 毎日体重を測って記録する習慣、これがとても大切です！",
        f"  • 体重と向き合う勇気、それが変化への第一歩ですよ",
    ]
    good_points.append(random.choice(measurement_compliments))

    # ランダムに2-3個選んで表示
    selected_points = random.sample(good_points, min(3, len(good_points)))
    for point in selected_points:
        print(point)

    print()

    # 改善ポイント（1つだけ、優しく、柔軟に）
    print("💡 改善ポイント（1つだけ！）:")
    improvement_tips = [
        "  • 有酸素運動、5分でも10分でもいいので取り入れてみましょう。完璧じゃなくていいんです。",
        "  • タンパク質を意識して摂ると、筋肉が維持されやすくなります。目安は守れなくても大丈夫。",
        "  • 水分補給も大切です。1日2リットルが理想ですが、できる範囲で増やしてみましょう。",
        "  • 睡眠時間も体づくりには重要です。7時間が理想ですが、6時間でも続けることが大事。",
        "  • 炭水化物を完全に抜くより、量を調整する方が続けやすいですよ。現場ではその方が結果が出ます。",
        "  • 食事の回数を分けると代謝が安定しやすい、と言われていますが、自分に合うやり方でOKです。",
        "  • ストレッチも忘れずに。完璧にやらなくても、少しやるだけで違います。",
        "  • 数字を気にしすぎないで。体の反応を見ながら調整していきましょう。",
        "  • 研究では○○と言われていますが、大事なのはあなたに合うかどうかです。",
        "  • 80点を目指しましょう。100点を狙うより、80点で続ける方が結果が出ます。",
    ]
    print(random.choice(improvement_tips))

    print()

    # 明日への一言
    print("🔥 明日への一言:")
    tomorrow_messages = [
        "  継続は力なり！明日も一緒に頑張りましょう！",
        "  小さな積み重ねが、大きな変化を生みます。明日も楽しみにしています！",
        "  あなたのペースで大丈夫。焦らず、着実に進みましょう！",
        "  今日も1日お疲れさまでした。明日もあなたらしく頑張りましょう！",
        "  完璧じゃなくていい。続けることが何より大切です！",
        "  変化は必ず訪れます。信じて続けましょう！",
        "  明日も、できることから始めましょう。応援しています！",
    ]
    print(random.choice(tomorrow_messages))

    print()
    print("=" * 60)


def main():
    """
    メイン関数：プログラムの全体の流れを制御
    """
    # ヘッダー表示
    print_header()

    # プラン選択
    plan = select_plan()

    print()  # 改行

    # プロフィール取得
    profile = get_profile()

    # プラン情報をプロフィールに統合
    profile.update(plan)

    # プロフィール確認
    show_profile_summary(profile)

    # 開始メッセージ
    print(f"\n🎉 {profile['name']}さん、一緒に頑張りましょう！")
    print(f"💪 {profile['diet_mode']}で目標体重{profile['target_weight']}kgを目指しましょう！")
    print(f"📅 毎日、全力でサポートします！\n")
    print("それでは、毎日の報告を始めましょう。")
    print("（いつでも「exit」または「終了」と入力すると終了できます）\n")


    # 体重履歴を記録するリスト
    weight_history = [profile["initial_weight"]]

    # 日々の記録を保存するリスト
    daily_records = []

    # 日々の報告ループ
    day_count = 1

    while True:
        print(f"\n{'='*60}")
        print(f"📅 {day_count}日目")
        print(f"{'='*60}")

        # 挫折リスクチェック（2日目以降）
        if day_count >= 2:
            risk_info = detect_dropout_risk(profile, weight_history, daily_records, day_count)
            provide_special_support(risk_info, profile)

        # 今日の入力を受け取る

        # 終了判定（ユーザーが途中で終了を選択した場合）
        if daily_data is None:
            print("\n" + "=" * 60)
            print(f"🎉 {profile['name']}さん、今日もお疲れさまでした！")
            print("=" * 60)
            print("\n継続は力なり！")
            print("また明日も一緒に頑張りましょう！💪")
            print("\n― ダイエットメンターより")
            print("=" * 60)
            break

        # 体重を履歴に追加
        weight_history.append(daily_data["weight"])

        # 日々の記録を保存
        daily_records.append(daily_data)

        # フィードバックを生成
        generate_feedback(profile, daily_data)

        # 週刊レポート（7日目、14日目、21日目...）
        if day_count % 7 == 0:
            week_number = day_count // 7
            print("\n\n" + "🎉" * 35)
            print()
            print(" " * 20 + "📬 週刊レポート 📬")
            print()
            print("🎉" * 35)

            # 週刊レポートを生成
            weekly_report = generate_weekly_report(
                profile, weight_history, daily_records, week_number
            )
            print(weekly_report)

            # レポートをファイルに保存
            filepath = save_report_to_file(
                weekly_report, profile["name"], "weekly", week_number
            )
            if filepath:
                print(f"📄 レポートを保存しました: {filepath}\n")


        # 月間レポート（30日目、60日目、90日目...）
        if day_count % 30 == 0:
            month_number = day_count // 30
            print("\n\n" + "🌟" * 35)
            print()
            print(" " * 20 + "📬 月間レポート 📬")
            print()
            print("🌟" * 35)

            # 月間レポートを生成
            monthly_report = generate_monthly_report(
                profile, weight_history, daily_records, month_number
            )
            print(monthly_report)

            # レポートをファイルに保存
            filepath = save_report_to_file(
                monthly_report, profile["name"], "monthly", month_number
            )
            if filepath:
                print(f"📄 レポートを保存しました: {filepath}\n")


        # 日数をインクリメント
        day_count += 1

        # 初期体重を更新（次回の比較用）
        profile["initial_weight"] = daily_data["weight"]


if __name__ == "__main__":
    # プログラムを実行
    main()
